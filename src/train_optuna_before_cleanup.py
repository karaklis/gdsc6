...
class CFG:
    ...
    phase = 'pretrain'  # 'pretrain', 'train', 'predict'
    pretrain_mode = True
    pretrain_labels = 10
    verbose = 1
    ...
    batch_size = 64
    epochs = 8 if phase == 'pretrain' else 20
    ...
    lr_min_factor = 1e-2        # for scheduler; 1 means constant LR, 1e-2 means final LR is 1/100 of initial LR
    ...
    prob_jitter = .5            # probability of adding sampling jitter
    ...
    spec_width = n_mels * 2     # width of spectrogram; 2 means square image, which is good for efficientnet b0
    time_step = int((time_focus * time_long ** 2) ** (1 / 3) + .5)  # time offset step forward for splitting up audios
...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.isnan(spec).any():  # detected nan
            # print(f'Nan in getitem at index {idx} in dataset with length {len(self.df)}')
            # print(f'item X:\n{self.df.loc[idx]}')
            # print(f'item spec:\n{spec}')
        ...
        if np.all(spec == 0.0) or np.all(spec == 1.0):
            # print(f'all elements 0 or 1 in getitem at index {idx} in dataset with length {len(self.df)}')
            # print(f'item X:\n{self.df.loc[idx]}')
            # print(f'item spec:\n{spec}')
            # idxa = self.df.at[idx, 'idx']
            # print(f'audio data at index {idxa}:\n{self.df_audio.loc[idxa]}')
    ...
    for epoch in range(CFG.epochs):
        ...
        if valid_dataloader:
            ...
            if val_loss < CFG.val_loss_best:
                ...
                if CFG.save_model_to_disk:
                    save_model(CFG.epochs, model, optimizer, criterion, epoch)
...
def get_audio():
    ...
    if CFG.pretrain_mode:
        ...
        print(f'reduced to {len(df_audio)} due to debug mode')
...
def objective(trial, train_dataloader=None, valid_dataloader=None):
    params = {  # description of each parameter in class CFG
        ...
        # step 2: optimizing time windows
...
def prediction():
    ...
    model = torch.load(f'{CFG.input_root}models/pytorch_buzz.pkl')
    criterion = nn.CrossEntropyLoss()
...
def prepare_dataset_test(df, df_audio):
    ...
    test_dataloader = DataLoader(
        test_dataset, batch_size=CFG.batch_size, shuffle=True, collate_fn=LoadDataset.collate_fn)
...
def prepare_dataset(df, df_audio, df_noise, df_ir):
    ...
    return train_df, valid_df, train_dataset, valid_dataset, train_dataloader, valid_dataloader
...
# prepare audio data for training and validation dataset
def preprocess_audio():
    ...
    _, test_idx = train_test_split(df_audio.index.values, test_size=0.3, stratify=df_audio['label'].values)
    df_audio['tts'] = 0
...
def pretraining():
    ...
    train_df, valid_df, train_dataset, valid_dataset, train_dataloader, valid_dataloader = \
...
def rejoin_chunks(df, df_audio, pred):
    print('1st attempt to rejoin')
    print(f'chunk data: {df.info()}:\n{df}')
    print(f'audio data: {df_audio.info()}:\n{df_audio}')
    print(f'prediction data: {pred.shape}:\n{pred}')
    cols = [str(x) for x in list(range(pred.shape[0]))]
    df_pred = pd.DataFrame(pred, columns=cols)
    df = pd.concat([df, df_pred], axis=0, ignore_index=True)
    print(f'combined df: {df}')
...
def save_model(epochs, model, optimizer, criterion, epoch):
    torch.save({
        ...
        'loss': criterion}, f"{CFG.input_root}data/model/model_pretrained_{epoch}.pth")


def save_spec(spec, idx, idxa):
    img = Image.fromarray(np.uint8(255.9999 - spec * 255.9999))
    img.save(f'{CFG.input_root}data/specs/spec_{CFG.time_long}_{CFG.time_focus}_{idxa}_{idx}.png')
...
def training():
    ...
    # visualize_history(f1, loss, val_f1, val_loss)
    # CFG.save_spectrogram = False
    torch.save(model, f'{CFG.input_root}models/pytorch_buzz.pkl')
...
def transform_audio(df, dfa, idx=0, train=True, noise_data=pd.DataFrame(), ir_data=pd.DataFrame()):
    ...
    audio_focus = np.tile(audio, 2)[int(focus_point * CFG.target_sr) : int((focus_point + CFG.time_focus) * CFG.target_sr)]
...
def update_cfg(params):
    ...
    CFG.time_step = (CFG.time_focus * CFG.time_long ** 2) ** (1 / 3)  # time offset step forward
