...
class CFG:
    ...
    # input_root = "../"
    suffix = '_1xx'
    model_location = f'data/models/effnet_buzz{suffix}.pkl'
    ...
    batch_size = 64
    epochs = 8  # if phase == 'pretrain' else 8  # 20
    ...
    lr_min_factor = 5e-2      # for scheduler; 1 means constant LR, 1e-2 means final LR is 1/100 of initial LR

    # audio specifics
    target_sr = 44100           # sampling rate
    # n_mels = 112              # height of spectrogram; 64-128 chosen 112 to match input size for efficientnet b0
    time_long = 1.25            # long time window, taken from original signal
    time_focus = 0.25            # focus point within long time window
    time_step = 1.25            # time offset step forward for splitting up longer recordings
    ...
    prob_jitter = .25           # probability of adding sampling jitter
    ...
    fmin = target_sr // 64      # minimum frequency
...
def rejoin_chunks(df, df_audio, pred):
    ...
    # df_details_before_rejoin.to_csv(f'{CFG.output_dir}pred_details_bf_rj{CFG.suffix}.csv', index=False)
    ...
    df_details.to_csv(f'{CFG.output_dir}pred_details{CFG.suffix}.csv', index=False)
    print(f'prepared for prediction:\n{df}')
    df[['file_name', 'predicted_class_id']].to_csv(
        f'{CFG.output_dir}prediction{CFG.suffix}.csv', index=False)
    print('done')
...
def save_stats(df, fn):
    df.loc[:, df.columns != 'audio'].to_csv(f'{CFG.output_dir}data/stats/{fn}.csv', index=False)
...
def transform_audio(df, dfa, idx=0, train=True, noise_data=pd.DataFrame(), ir_data=pd.DataFrame()):
    ...
    spec = get_spectrogram(audio, width=224, height=224)
