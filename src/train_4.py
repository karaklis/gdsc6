...
class CFG:
    ...
    batch_size = 64
    epochs = 8 if phase == 'pretrain' else 16  # 20
    ...
    lr_min_factor = 4.5e-2      # for scheduler; 1 means constant LR, 1e-2 means final LR is 1/100 of initial LR

    # audio specifics
    target_sr = 44100           # sampling rate
    # n_mels = 112              # height of spectrogram; 64-128 chosen 112 to match input size for efficientnet b0
    time_long = 16.0            # long time window, taken from original signal
    time_focus = 1.0            # focus point within long time window
    time_step = 15.0            # time offset step forward for splitting up longer recordings
...
def rejoin_chunks(df, df_audio, pred):
    ...
    df_details_before_rejoin.to_csv(f'{CFG.output_dir}pred_details_before_rejoin.csv', index=False)
