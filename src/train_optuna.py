...
class CFG:
    ...
    min_sample = 80             # upsampling threshold
    ...
    time_long = 60.0            # long time window, taken from original signal
    time_focus = 0.6            # focus point within long time window
    time_step = 12.0             # time offset step forward for splitting up longer recordings
...
def calculate_weights(df):
    ...
    # print(f'weights: {w.cnt.values.flatten()}')
...
def rejoin_chunks(df, df_audio, pred):
    ...
    # print(f'prediction data: {p.shape}\n{p}')
    ...
    # print(f'aggregated df:\n{df}')
    df['predicted_class_id'] = df[cols].idxmax(axis='columns')
    # print(f'with predicted label:\n{df}')
    ...
    df_details.to_csv(f'{CFG.input_root}predictions/pred_details.csv', index=False)
    # print(f'prepared for prediction:\n{df}')
    df[['file_name', 'predicted_class_id']].to_csv(f'{CFG.input_root}predictions/prediction.csv', index=False)
    print('done')
