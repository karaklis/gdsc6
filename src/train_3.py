...
class CFG:
    ...
    model_location = 'data/models/effnet_buzz.pkl'
...
def get_spectrogram(audio, height, width):
    ...
    return medfilt2d(spec, kernel_size=3)
...
def rejoin_chunks(df, df_audio, pred):
    ...
    df_details.to_csv(f'{CFG.output_dir}pred_details.csv', index=False)
    df[['file_name', 'predicted_class_id']].to_csv(f'{CFG.output_dir}prediction.csv', index=False)
    print('done')

def save_spec(spec, idx, idxa, offset):
    img = Image.fromarray(np.uint8(spec * 255.9999))
...
# split spectrogram into four octaves, then merge these into rgb
def split_rgb(s):
    h, w = s.shape  # width and height of spectrogram
    h = h // 4  # height for each octave
    s = s[:h*4, :]  # in case the height is not divisible by 4
    s1, s2, s3, s4 = s[:h, :], s[h:2*h, :], s[2*h:3*h, :], s[3*h:, :]
    # print(s.shape, s1.shape, s2.shape, s3.shape, s4.shape)
    sr, sg, sb = s1 * .75 + s2 * .25, s2 * .5 + s3 * .5, s3 * .25 + s4 * .75
...
def transform_audio(df, dfa, idx=0, train=True, noise_data=pd.DataFrame(), ir_data=pd.DataFrame()):
    ...
    spec = split_rgb(get_spectrogram(audio, width=224, height=32*4))
    spec1 = split_rgb(get_spectrogram(audio_focus1, width=112, height=96*4))
    spec2 = split_rgb(get_spectrogram(audio_focus2, width=112, height=96*4))
    spec3 = split_rgb(get_spectrogram(audio_focus3, width=112, height=96*4))
    spec4 = split_rgb(get_spectrogram(audio_focus4, width=112, height=96*4))
    # print(spec.shape, spec1.shape, spec2.shape, spec3.shape, spec4.shape)
    # if np.isnan(spec).any():
    ...
    spec12 = np.concatenate([spec1, spec2], axis=2)
    spec34 = np.concatenate([spec3, spec4], axis=2)
    # print(spec.shape, spec12.shape, spec34.shape)
    spec = np.concatenate([spec, spec12, spec34], axis=1)
    spec = np.transpose(spec, (1, 2, 0))
    # print(spec12.shape, spec34.shape, spec.shape)
